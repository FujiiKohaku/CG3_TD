...
void Input::Update()
{
	...
	if (conResult == ERROR_SUCCESS) {
		...
		if (xinputState_.Gamepad.wButtons & XINPUT_GAMEPAD_B) { isPushB_ = true; }
		...
		if (xinputState_.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT) { isPushLeft_ = true; }
		if (xinputState_.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT) { isPushRight_ = true; }
